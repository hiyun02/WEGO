...
public class NoticeService implements INoticeService {
    ...
    public Page<NoticeDTO> getNoticeList(Pageable pageable) {
        ...
        // 공지사항 전체 리스트 조회하기
        Page<NoticeEntity> rList = noticeRepository.findAllByOrderByNoticeSeqDesc(pageable);

        log.info("rList size : "+rList.getSize());
        // 엔티티의 값들을 DTO에 맞게 넣어주기
        Page<NoticeDTO> nList = rList.map(NoticeDTO::toDTO);
    ...
    public void insertNoticeInfo(NoticeDTO pDTO) throws Exception {
        ...
        NoticeEntity pEntity = NoticeEntity.builder()
                ...
                .regId(userId).regDt(DateUtil.getDateTime("yyyy.MM.dd"))
                .chgId(userId).chgDt(DateUtil.getDateTime("yyyy.MM.dd"))
                .build();
